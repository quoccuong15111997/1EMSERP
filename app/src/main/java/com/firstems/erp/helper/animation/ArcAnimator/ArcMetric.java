package com.firstems.erp.helper.animation.ArcAnimator;
...
public class ArcMetric {
    ...
    private void calcStartEndSeg() {
        mStartEndSegment =
                (float) Math.sqrt(Math.pow(mStartPoint.x - mEndPoint.x, 2) +
    ...
    private void calcRadius() {
        ...
        mRadius =
                mStartEndSegment / ArcUtils.sin(mAnimationDegree) * ArcUtils.sin(mSideDegree);
    }

    private void calcMidAxisSeg() {
        mMidAxisSegment = mRadius * ArcUtils.sin(mSideDegree);
    }
    ...
    private void calcDegrees() {
        mZeroStartSegment =
                (float) Math.sqrt(Math.pow(mZeroPoint.x - mStartPoint.x, 2) +
                        Math.pow(mZeroPoint.y - mStartPoint.y, 2));
        mZeroStartDegree =
                ArcUtils.acos((2 * Math.pow(mRadius, 2) - Math.pow(mZeroStartSegment, 2)) / (2 * Math.pow(mRadius, 2)));
        switch (mSide) {
            case RIGHT:
    ...
    public float getDegree(float percentage) {
